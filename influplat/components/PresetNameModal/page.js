...
export default function PresetNameModal({ modalFlag, setModalFlag, presetName, setPresetName, preset, setPreset, storeDoc}){
    ...
    return(
        <div className={`absolute top-0 left-0 min-h-screen w-screen flex flex-col items-center text-black ${modalFlag===true?'block':'hidden'}`}>
            <div className="bg-zYellow-500 w-full md:w-fit h-auto flex flex-col items-center px-4 py-8 border-2 border-black">
                ...
                />
                <div className={`bg-zGreen-500 border-2 shadow-harsh5px hover:shadow-harsh10px border-softBlack-500 mx-4 my-4 flex flex-col items-center font-bold text-md hover:bg-zPink-500 transition-all duration-300 text-xl active:scale-90 active:shadow-harsh5px`} onClick={btnHandler}>
                    Save Preset
                </div>
            </div>
        </div>

export default function PresetComponent({ loggedIn, data, presetToLoad, setPresetToLoad, presetFlag, setPresetFlag, presetToDelete, setPresetToDelete, deletePreset, setDeletePreset }){
    ...
    return(
        <div className={`px-4 py-2 m-2 max-w-fit font-opensans text-zGreen-900 w-full bg-zGreen-500 border-2 border-zGreen-900 hover:border-zGreen-500 hover:text-zGreen-500 hover:bg-zGreen-900 transition-all duration-300 hover:rounded-3xl flex flex-wrap items-center ${loggedIn?'block':'hidden'}`} onClick={handleClick}>
            {data.id}
